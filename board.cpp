...
static int tile_compare(const void* generic_tile_1, const void* generic_tile_2)
{
...
static RRTile* tile_search(const RRTile* tile_start, unsigned int tile_size, const RRTile* query)
{
    return (RRTile*) std::bsearch(query, tile_start, tile_size, sizeof(RRTile), tile_compare);
}

/* recovering neighboring tiles */

static const RRTile* tile_get_N(const RRBoard* board, const RRTile* query /* must belong to the board */)
{
    RRTile key;
...
static const RRTile* tile_get_S(const RRBoard* board, const RRTile* query /* must belong to the board */)
{
    RRTile key;
    key.line = query->line + 1;
    key.column = query->column;
...
static const RRTile* tile_get_W(const RRBoard* board, const RRTile* query /* must belong to the board */)
{
...
static const RRTile* tile_get_E(const RRBoard* board, const RRTile* query /* must belong to the board */)
{
...
static const RRTile* tile_get(const RRBoard* board, const RRTile* query, Direction direction)
{
    switch(direction)
    {
...
int tile_is_fast(const RRTile* tile) 
{
    return 
        tile && 
...
Direction tile_direction(const RRTile* tile)
{
...
Rotation tile_rotation(const RRTile* tile, Direction direction /* some tiles depend on the incoming direction */)
{
    /* static tiles */
...
static const RRTile* robot_locate(const RRTile* tiles, unsigned int tile_size, const RRRobot* robot)
{
    RRTile key;
    key.line = robot->line;
...
static void robot_move(RRRobot* robot, Direction direction)
{
...
static void robot_rotate(RRRobot* robot, Rotation rotation)
{
...
/* tile has to belong to the board */
static const RRTile* robot_apply(RRRobot* robot, const RRBoard* board, const RRTile* tile)
{
    if(tile)
    {
...
void rr_board_init (RRBoard& board_in, const char* filename)
{
    ...
    if(!board)
    {
        ...
        throw GenEx(ExNullPointer, "board ne peut pas etre null");
        return;
    }
        ...
        if(tile_size == alloc_size)
        {
            ...
            if(!tmp) 
            {
                ...
                if(!tmp)
                {
                    ...
                    fclose(board_file);
                    board->tiles = NULL;
                    throw GenEx(ExMemory, "Impssible de reallouer de la memoire pour le plateau");
                }
                else
                {
    ...
    if(board->tile_size)
    {
        ...
        for(RRTile* t = board->tiles + 1; t < board->tiles + board->tile_size; ++t)
        {
        ...
        for(RRTile* t = board->tiles; t < board->tiles + board->tile_size; ++t)
        {
            t->line   -= lmin;
            t->column -= cmin;
        }
...
static void board_apply(const RRBoard* board, RRRobot* robot)
{
    /* a dead robot does not move (OS dependent) */
    if(robot->status == RR_ROBOT_DEAD) return;
...
void rr_board_play(const RRBoard& board, RRRobot& robot, RRRobotMove move)
{
